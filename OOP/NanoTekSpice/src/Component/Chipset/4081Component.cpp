...
void C4081::simulate(size_t tick)
{
    ...
    if (this->_links.size() == 0)
        throw PinError("C4081 simulate: invalid link\n");

    if (tick == 2) {
        this->setPinValue(2, lg.ANDGate(this->getValuePin(0), this->getValuePin(1)));
    }
    if (tick == 3) {
        this->setPinValue(3, lg.ANDGate(this->getValuePin(4), this->getValuePin(5)));
    }
    if (tick == 9) {
        this->setPinValue(9, lg.ANDGate(this->getValuePin(8), this->getValuePin(7)));
    }
    if (tick == 10) {
        this->setPinValue(10, lg.ANDGate(this->getValuePin(11), this->getValuePin(12)));
    }
