...
rl::Texture::Texture(const std::string message, rl::Font font, int size, rl::Color color)
{
    _texture = LoadTextureFromImage(ImageTextEx(font.getFont(), message.c_str(), size, 1, color));
    SetTextureFilter(_texture, TEXTURE_FILTER_ANISOTROPIC_16X);
}
...
void rl::Texture::drawBillboard(rl::Camera3d cam, rl::Vec3 pos, float size, rl::Color color)
{
    DrawBillboard(cam, _texture, pos, size, color);
}